...
public class BookController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		 ...
		 *  * 부분에 추가된 subPath를 추출하여 여러가지 요청을 
		 *  처리할수 있다
		 *  이때 추가된 subPath를 추출할때는 getPathInfo()를
		 *  사용하여 추출한다
		 */
	...
	private void callService() {
		...
		bookVO.setBk_ccode("C00001");
		bookVO.setBk_acode("A00001");
		...
		// 도서정보 1개 조회
		BookVO retVO = bService.findById("97000000000");
		// 도서정보 변경
		bService.update(bookVO);
		// 도서 데이터 1개 삭제
		bService.delete("97000000000");
	}
