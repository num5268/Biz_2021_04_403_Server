...
public class BookRentServiceImplV1 implements BookRentService{
	 ...
	 */
	protected List<BookRentDTO> select(PreparedStatement pStr) 
			throws SQLException{
	...
	public BookRentDTO findById(Long seq) {
		 ...
		 * 만약 매개변수로 직업 SQL 명령문을 다음과 같이 만들면
		 * " WHERE 도서명 = " + title;
...
//			pStr.close();
			// PK로 조회를 했기 때문에
			// List에는 1개밖에 데이터가 없다
			// List의 0번 데이터만 getter 하여 DTO에 담기
			List<BookRentDTO> brList = this.select(pStr);
	...
	@Override
	public List<BookRentDTO> findByBName(String name) {
		// TODO 도서명으로 조회
		String sql = " SELECT * FROM VIEW_도서대여정보 ";
		sql += " WHERE 도서명 LIKE '%' || ? || '%' ";
