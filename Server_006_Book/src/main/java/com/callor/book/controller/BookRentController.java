	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		if(subPath.equals("/seq")) {
			...
			if(strSeq == null || strSeq.equals("")) {
				 ...
				 * 요청(Req) 응답(Res)를 총괄하는 정보가 담긴 객체
				 * Web App Service를 구현하기 위하여
				 * Req, Res를 처리하는 여러가지 기능을 구현해야 하는데
				 * 그러한 기능을 미리 구현해 놓았기 때문에
				 * ServletContext를 getter 하는 것만으로 충분하다
				 * 
				 * DB 등으로 부터 조회된 데이터를 Web에게
				...
				// BookRentDTO BOOK = 
				//		brDTO 이런형식의 코드가 실행된다
				// 세팅된 BOOK 객체변수는 jsp파일에서 참조하여
				//		값을 표현할수 있다
				app.setAttribute("BOOK", brDTO);
				
				// book.jsp파일을 읽어서 
				// app에 setting한 BOOK변수와 함께
				// Rendering을 하라
