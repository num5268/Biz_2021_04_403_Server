...
/*
	...
	항상 제일먼저 있어야 할 설정
	기본 HTML의 margin, padding을 clear(Reset)아여
...
/*
	...
	nav#main ul{ }	
	id가 main인 nav Box 내부에 있는 ul tag에만
	제한적으로 적용하라 하는 의미로 사용한다
	본문에 다른 곳에 있는 ul에는 속성을 적용하지 말라
*/
	nav#main ul{
		margin:0px;
...
<h1 class="main">방명록 2021</h1>
	...
	<nav id="main">
		<ul>
			<%-- 
			일반적으로 메뉴(네비게이션)등을 만들때
			해당 메뉴를 클릭하면 다른 page로 점프하는 방싱을
			가장 쉽게 구현하기 위하여 a tag를 사용한다
			
			a tag를 사용할때 a tag의 기본속성을 제거하고 
			새로운 속성을 부여하는 css 코드를 추가 한다 
			
			이러한 방식이 번거로워서 아예 a tag를 사용하지
			않고 다른 종류의 tag를 사용하여 메뉴를 구성하기도
			한다
			
			여기서는 ul>li tag를 조합하여 상단의 메뉴를 
			생성하였다
			...
			함에도 불구하고 a tag 사용을 하지않는 방식을 많이
			사용한다
			
			스크립트를 사용하면 또 다른 이점이 있는데
			메뉴와 관련된 부분을 스크립트에서 모아서 
			사용할 수 있고
			상대적으로 html 코드에 직접 anchor를 설정하는 것보다
			본문코드가 간편해지기도 한다 
			--%>
